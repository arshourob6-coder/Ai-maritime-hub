...
import { Play, RotateCcw, Compass, Anchor, Navigation, Activity, Wind } from 'lucide-react';
...
export const MarineSimulatorsView: React.FC<Props> = ({ userPlan = 'student', onOpenPricing, onSelectView }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6 text-slate-100">
      ...
      {/* Mode Selector */}
      <div className="flex bg-slate-900 p-1 rounded-2xl border border-slate-800 text-xs font-bold shadow w-fit">
        <button
          onClick={() => setActiveTab('offshore_wind')}
          className="px-4 py-2 rounded-xl transition flex items-center gap-2 text-slate-400 hover:text-white"
        >
          <Wind className="w-4 h-4 text-cyan-300" />
          <span>Offshore Wind SCADA Telemetry</span>
        </button>
        <button
          onClick={() => setActiveTab('simulator')}
          className="px-4 py-2 rounded-xl transition flex items-center gap-2 bg-teal-600 text-white shadow"
        >
          <Compass className="w-4 h-4 text-teal-300" />
          <span>6-DOF Bridge & Engine Maneuvering Sim</span>
        </button>
